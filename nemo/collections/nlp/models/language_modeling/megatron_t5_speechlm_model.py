...
from nemo.collections.nlp.data.language_modeling.megatron.t5_mm_sft_dataset import MMT5SFTDataset
...
class MegatronT5SpeechLMModel(MegatronBaseSpeechLM):
    ...
    def forward(
        ...
        if self.first_stage_of_pipeline():
            ...
            _input_tokens = [context_and_question_tokens]
            if virtual_tokens is not None:
                _input_tokens = [virtual_tokens, context_and_question_tokens]

            input_embeds = self.get_embeddings_and_combine(
                _input_tokens, taskname_ids, inference
            )
    ...
    def build_recognition_multitask_dataset(
        self, file_paths, concat_sampling_probabilities, batch_size, for_train, drop_last, shuffle, num_workers, pin_memory
    ):
        # import ipdb; ipdb.set_trace()
        if for_train:
            data_prefix = []
            for weight, prefix in zip(concat_sampling_probabilities, file_paths):
            ...
            num_train_samples_per_dataset = [[None]] * len(file_paths)

        datasets = []
        for file_path, num_samples in zip(file_paths, num_train_samples_per_dataset):
            _dataset = MMT5SFTDataset(
                file_path=file_path,
                tokenizer=self.tokenizer,
                max_seq_length=self.cfg.data.get('max_seq_length', self.frozen_model.cfg.max_position_embeddings),
                min_seq_length=self.cfg.data.get('min_seq_length', 1),
                add_bos=self.cfg.data.get('add_bos', False),
                add_eos=self.cfg.data.get('add_eos', True),
                add_sep=self.cfg.data.get('add_sep', True),
                sep_id=self.tokenizer.sep_id,
                max_num_samples=num_samples[0],
                seed=self.cfg.seed,
                label_key=self.cfg.data.get('label_key', 'answer'),
                answer_only_loss=self.cfg.get('answer_only_loss', True),
                truncation_field=self.cfg.data.get('truncation_field', 'context'),
                pad_to_max_length=self.cfg.data.get('pad_to_max_length', False),
                index_mapping_dir=self.cfg.data.get('index_mapping_dir', None),
                prompt_template=self.cfg.data.get('prompt_template', None),
                virtual_tokens=0,
                tokens_to_generate=self.cfg.data.get(
                    'tokens_to_generate', 0
                ),  # used at inference time to allocate tensor positions for tokens that will be generated by inf procedure.
                memmap_workers=self.cfg.data.get(
                    'memmap_workers', None
                ),  # used to set num. of workers to create the memmap index files
                hf_dataset=self.cfg.data.get(
                    'hf_dataset', False
                ...
                truncation_method=self.cfg.data.get(
                    'truncation_method', 'right'
                ),  # used to choose truncation method. Options: ['random', 'left', 'right']
                num_audio_codebooks=self.cfg.data.get('num_audio_codebooks', 8),
                audio_codebook_size=self.speech_codebook_size,
                audio_token_offset=self.speech_offset,
                pad_audio_to_length=self.cfg.data.get('pad_audio_to_length', 0),
                attn_mask_type=self.cfg.data.get('attn_mask_type', 'causal'),
