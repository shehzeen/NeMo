...
class MegatronT5SpeechLMModel(MegatronBaseSpeechLM):
    ...
    def get_forward_output_and_loss_func(self):
        def fwd_output_and_loss_func(dataloader_iter, model):
            ...
            print(f"context_and_question_tokens_shape:  {context_and_question_tokens.shape}")
            print(f"labels_shape:  {labels.shape}")
            # @shehzeen print text from context_and_question_tokens
            # context_and_question_tokens (B, 8, S)
            # print ("0th layer: ",  context_and_question_tokens[0,0,:])
            # print ("1st layer: ", context_and_question_tokens[0,1,:])
            # Decode the text from context_and_question_tokens
            token_list = [context_and_question_tokens[0,0,i].item() for i in range(context_and_question_tokens.shape[2])] 
            token_list = [t for t in token_list if t != 0 and t < 30000]
            input_text = self.frozen_model.tokenizer.ids_to_text(token_list)
            print ("0th layer: " , input_text)
