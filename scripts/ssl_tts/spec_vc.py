...
def main():
    ...
    fastpitch_model = fastpitch_spec.FastPitchModel_SSL.load_from_checkpoint(args.fastpitch_ckpt_path, strict=False)
    ...
    for source_target_out in source_target_out_pairs:
        ...
        with torch.no_grad():
            audio = load_wav(source_audio_path, wav_featurizer).to(device)
            audio_len = torch.tensor(audio.shape[0]).to(device)
            content_embedding1, _ = fastpitch_model.audio_to_melspec_precessor(audio[None], audio_len[None])
            duration1 = torch.ones(content_embedding1.shape[2])[None]
